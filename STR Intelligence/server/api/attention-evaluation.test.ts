...
test("evaluates every supplied home and returns safe explanations", async () => {
  ...
  const evaluations = result.body.evaluations as Array<{ listingIndex: number; result: { attentionScore: number | null; confidenceScore: number | null }; explanation: { summary: string }; priority: { band: string } }>;
  ...
  assert.ok(["review_now", "promising", "low_priority", "ineligible"].includes(evaluations[0]?.priority.band ?? ""));
...
test("returns an unscorable result instead of failing the batch when price is missing", async () => {
  ...
  assert.equal(result.statusCode, 200);
  const evaluation = (result.body.evaluations as Array<{ result: { attentionScore: null; evaluability: { status: string } }; priority: { band: string } }>)[0];
  assert.equal(evaluation?.result.attentionScore, null);
  assert.equal(evaluation?.result.evaluability.status, "unscorable");
  assert.equal(evaluation?.priority.band, "ineligible");
});
