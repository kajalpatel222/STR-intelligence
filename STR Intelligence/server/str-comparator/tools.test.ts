...
function repository(): StrComparisonRepositoryPort {
  return {
    ...
    async findComparisonCache(canonicalPropertyId) {
...
test("defines named LangChain tools and invokes injected deterministic capabilities", async () => {
  ...
  assert.equal((await tools.lookupComparisonCache.invoke({ canonicalPropertyId: "property-1" })).status, "fresh");
  await tools.discoverNearbyStays.invoke({ location: "Oakhurst, CA", limit: 15, currency: "USD", locale: "en-US", checkIn: "2026-09-11", checkOut: "2026-09-13" });
