...
export function toPersistedEvidence(calendar: ListingCalendar, rawPayload?: unknown): readonly PersistedEvidence[] { const valid = normalizedCalendarDays(calendar); if (!valid.length) return []; const calendarWindow = valid.slice(0, 90); const available = calendarWindow.filter((day) => day.available).length; const unavailable = calendarWindow.length - available; return [Object.freeze({ providerListingKey: calendar.listingId, rates: Object.freeze(valid.flatMap((day) => positive(day.nightlyRate) ? [{ date: day.date, nightlyRateUsd: day.nightlyRate!, rawEvidence: day }] : [])), calendar: Object.freeze({ windowStart: calendarWindow[0]!.date, windowEnd: calendarWindow.at(-1)!.date, availableNights: available, unavailableNights: unavailable, unknownNights: 0, unavailabilityRate: unavailable / calendarWindow.length, dailyObservations: calendarWindow, rawEvidence: rawPayload }), observedAt: calendar.scrapedAt ?? new Date().toISOString() })]; }
