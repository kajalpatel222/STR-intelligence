...
import { calculateComparatorSummary, type ComparatorCandidate, type ComparatorRadiusMiles, type ComparatorSummary } from "../../shared/str-comparator.js";
...
export type StrComparatorWorkflowState = Readonly<{
  ...
  dataOrigin?: "fresh_cache" | "market_database" | "saved_fallback";
...
export function initializeStrComparatorWorkflowState(params: {
  ...
  return Object.freeze({ workflowId: params.workflowId, intent: params.intent, listingUrl: params.listingUrl, radiusMiles: params.radiusMiles ?? 1, bypassCache: params.bypassCache ?? false,
...
export function createStrComparatorGraph(dependencies: { provider: StrComparatorProvider; repository: StrComparisonRepositoryPort }) {
  ...
  const lookupCache = async ({ workflowState }: typeof ComparatorGraphState.State) => {
    const cacheLookup = await tools.lookupComparisonCache.invoke({ canonicalPropertyId: workflowState.target!.canonicalPropertyId, radiusMiles: workflowState.radiusMiles });
  ...
  const collectProvider = async ({ workflowState }: typeof ComparatorGraphState.State) => {
    ...
    try {
      const providerBatch = await tools.discoverNearbyStays.invoke({
        location: [workflowState.target?.city, workflowState.target?.state].filter(Boolean).join(", "),
        latitude: workflowState.target!.latitude,
        longitude: workflowState.target!.longitude,
        radiusMiles: workflowState.radiusMiles,
        limit: 500,
        currency: "USD",
        locale: "en-US",
...
async function persistDiscoveredComparables(state: StrComparatorWorkflowState, batch: ProviderBatch<DiscoveredListing>, dependencies: { provider: StrComparatorProvider; repository: StrComparisonRepositoryPort }): Promise<StrComparatorWorkflowState> {
  try {
    ...
    const selected = Object.freeze(normalized
      .filter((candidate) => candidate.distanceMiles <= state.radiusMiles)
      .map((candidate) => Object.freeze({ ...candidate, similarityScore: similarityFor(target, candidate), matchReasons: Object.freeze(matchReasons(target, candidate)) }))
      .sort((left, right) => left.distanceMiles - right.distanceMiles || right.similarityScore - left.similarityScore));
    ...
    const requestSnapshot = { source: "airbtics_market", location: [target.city, target.state].filter(Boolean).join(", "), radiusMiles: state.radiusMiles, candidateLimit: 500, selectedCount: selected.length };
    const runId = await dependencies.repository.createRun({ target, publicReference, cacheKey: `${target.listingSnapshotId}:v1:${state.radiusMiles}mi:${dates.checkIn}`, request: requestSnapshot });
    const summary = calculateComparatorSummary({ target: toComparatorTarget(target), candidates: selected.map((candidate) => toComparatorCandidate(candidate, undefined, target)) });
    await dependencies.repository.saveDiscovery(runId, selected, summary);
    return finish({ ...state, target, runId, comparisonReference: publicReference, candidates: selected, summary, status: batch.errors.length ? "partial" : "completed", dataOrigin: "market_database" });
  } catch {
...
async function enrichComparables(state: StrComparatorWorkflowState, dependencies: { provider: StrComparatorProvider; repository: StrComparisonRepositoryPort }): Promise<StrComparatorWorkflowState> {
  try {
    ...
    const summary = calculateComparatorSummary({ target: toComparatorTarget(stored.target), candidates: selected.map((item) => toComparatorCandidate(item, calendarByKey.get(item.providerListingKey), stored.target)) });
...
function toPersistedCandidate(record: DiscoveredListing, target: ComparatorTargetRecord, dates: { checkIn: string; checkOut: string }, rawPayload?: unknown): readonly PersistedComparable[] {
  ...
  if (!entireHome || !record.listingId || !record.url || (!positive(record.nightlyRate) && !positive(record.adrLtmUsd)) || !coordinate(record.latitude, -90, 90) || !coordinate(record.longitude, -180, 180)) return [];
  ...
  return [Object.freeze({ providerSource: record.dataSource ?? "apify", providerListingKey: record.listingId, listingUrl: record.url, title: record.title, imageUrl: record.imageUrl,
    ...
    isSuperhost: record.isSuperhost, amenities: Object.freeze([...(record.amenities ?? [])]),
    observedNightlyPriceUsd: positive(record.nightlyRate) ? record.nightlyRate : undefined,
    observedCheckIn: positive(record.nightlyRate) ? dates.checkIn : undefined,
    observedCheckOut: positive(record.nightlyRate) ? dates.checkOut : undefined,
    adrLtmUsd: positive(record.adrLtmUsd) ? record.adrLtmUsd : undefined,
    occupancyLtmPercent: nonNegative(record.occupancyLtmPercent) ? record.occupancyLtmPercent : undefined,
    annualRevenueLtmUsd: nonNegative(record.annualRevenueLtmUsd) ? record.annualRevenueLtmUsd : undefined,
    marketCollectedAt: record.marketCollectedAt, similarityScore: 0, matchReasons: Object.freeze([]),
    observedAt: record.scrapedAt ?? new Date().toISOString(), rawPayload: rawPayload ?? {}, })];
}
...
function toComparatorCandidate(candidate: PersistedComparable, calendar?: ListingCalendar, target?: ComparatorTargetRecord): ComparatorCandidate { const days = calendar ? normalizedCalendarDays(calendar) : undefined; const targetFacts = target ? toComparatorTarget(target) : undefined; return { id: candidate.providerListingKey, name: candidate.title, entireHome: true, coordinates: { latitude: candidate.latitude, longitude: candidate.longitude }, bedrooms: candidate.bedrooms ?? targetFacts?.bedrooms ?? 0, bathrooms: candidate.bathrooms ?? targetFacts?.bathrooms ?? 0, accommodates: candidate.guestCapacity ?? targetFacts?.accommodates ?? 1, advertisedNightlyRateUsd: candidate.observedNightlyPriceUsd ?? candidate.adrLtmUsd ?? 1, rateObservations: days?.flatMap((day) => positive(day.nightlyRate) ? [{ date: day.date, nightlyRateUsd: day.nightlyRate! }] : []), calendarObservations: days?.slice(0, 90).map((day) => ({ date: day.date, availability: day.available ? "available" as const : "unavailable" as const })) }; }
...
function similarityFor(target: ComparatorTargetRecord, candidate: PersistedComparable) { const differences = [target.bedrooms !== undefined && candidate.bedrooms !== undefined ? Math.abs(target.bedrooms - candidate.bedrooms) * 2 : 0, target.bathrooms !== undefined && candidate.bathrooms !== undefined ? Math.abs(target.bathrooms - candidate.bathrooms) * 1.5 : 0, candidate.distanceMiles * .1]; return round((1 / (1 + differences.reduce((sum, value) => sum + value, 0))) * 100, 2); }
