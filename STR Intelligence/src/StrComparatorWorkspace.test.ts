...
test("renders the first paginated set of database-backed comparable cards", () => {
  const markup = renderToStaticMarkup(createElement(StrComparatorWorkspace, {
    listingUrl: "https://example.com/target",
  ...
  assert.equal((markup.match(/<article/g) ?? []).length, 6);
  ...
  assert.match(markup, /Search radius/);
  for (const radius of ["1 mile", "2 miles", "5 miles", "10 miles"]) assert.match(markup, new RegExp(radius));
...
test("shows Superhost and distance as separate compact facts", () => {
  ...
  assert.match(markup, /Distance<\/dt>/);
  assert.match(markup, /7818.1 mi/);
});

test("shows saved Airbtics LTM metrics without legacy calendar controls", () => {
  const value = comparison();
  const markup = renderToStaticMarkup(createElement(StrComparatorWorkspace, {
    listingUrl: "https://example.com/target",
    initialComparison: { ...value, candidates: [{ ...value.candidates[0]!, observedNightlyPriceUsd: undefined, adrLtmUsd: 248, occupancyLtmPercent: 72.4, annualRevenueLtmUsd: 54_000, marketCollectedAt: "2026-09-06T12:00:00Z" }] },
    onBack: () => undefined,
  }));
  assert.match(markup, /LTM ADR/);
  assert.match(markup, /LTM occupancy/);
  assert.match(markup, /LTM revenue/);
  assert.match(markup, /72.4%/);
  assert.match(markup, /\$54,000/);
  assert.equal(markup.includes("Calendar window"), false);
});
...
test("typed client sends stage-specific payloads without external calls", async () => {
  ...
  await discoverStrComparables("https://example.com/target", 10, fakeFetch);
  ...
  assert.deepEqual(JSON.parse(String(calls[0]!.init!.body)), { listingUrl: "https://example.com/target", radiusMiles: 10 });
...
test("normalizes the richer server response into the local UI DTO", async () => {
  ...
  const result = await discoverStrComparables("https://www.zillow.com/homedetails/123", 5, fakeFetch);
...
test("typed client surfaces server errors and rejects incomplete success responses", async () => {
  ...
  await assert.rejects(discoverStrComparables("target", 5, failedFetch), /Provider timed out/);
  const incompleteFetch = async () => new Response(JSON.stringify({ status: "complete" }), { status: 200 });
  await assert.rejects(discoverStrComparables("target", 5, incompleteFetch), /incomplete response/);
});
