...
import {
  formatBedroomBathroomCount,
  formatComparatorCurrency,
  formatComparatorDate,
  formatComparatorDistance,
  formatComparatorPercent,
} from "./str-comparator-format.js";
    ...
    <main className="str-comparator" style={styles.workspace} aria-labelledby="comparator-title" aria-busy={activity !== "idle"}>
      ...
      {comparison && <>
        ...
        {comparison.candidates.length === 0 ? <div style={styles.empty} role="status">
          ...
          {comparison.candidates.slice(0, 5).map((candidate, index) => {
            return <article key={candidate.providerListingKey ?? candidate.listingUrl} className="str-comparator__comparable" style={styles.card} onClick={(event) => openListingFromCard(event, candidate.listingUrl)}>
              ...
              <div className="str-comparator__identity">
                <div style={styles.cardTop}>
                  ...
                  {candidate.isSuperhost && <span className="str-comparator__superhost">Superhost</span>}
                </div>
                <div className="str-comparator__title-row">
                  <h3 style={styles.cardTitle}><a href={candidate.listingUrl} target="_blank" rel="noreferrer">{candidate.title ?? `Comparable ${index + 1}`}</a></h3>
                </div>
                ...
                <details style={styles.details}>
                  ...
                  <p>{candidate.matchReasons.length ? candidate.matchReasons.join(" · ") : "Ranked by distance and property similarity."}</p>
                </details>
              </div>
              <aside className="str-comparator__evidence">
                <div style={styles.observedPrice}>
                  <span>Current rate</span>
                  <strong>{formatComparatorCurrency(candidate.observedNightlyPriceUsd)}<small>/night</small></strong>
                </div>
                {hasCalendarMetric(candidate) && <div className="str-comparator__booking-metric">
                  <span>Booked or blocked</span>
                  <strong>{formatComparatorPercent(candidate.calendarUnavailablePercentage)}</strong>
                  <small>Based on {candidate.calendarObservationCount} observed calendar nights</small>
                </div>}
              </aside>
            </article>;
          })}
...
// Comparator discovery is market-bounded; suppress legacy cached mileage produced from the invalid (0, 0) target coordinate.
function credibleDistance(distanceMiles: number) { return Number.isFinite(distanceMiles) && distanceMiles >= 0 && distanceMiles <= 100 ? formatComparatorDistance(distanceMiles) : "Unavailable"; }
function hasCalendarMetric(candidate: StrComparisonDto["candidates"][number]) {
